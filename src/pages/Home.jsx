...
// ── Feature card ──────────────────────────────────────────────────────────────
const FeatureCard = ({ icon: Icon, title, desc, to, gradient, badge, delay }) => {
    const ref = useRef(null);
        ...
        >
            <Link to={to}>
                <div className="group relative bg-bg-card border border-border/60 rounded-3xl p-7 hover:shadow-xl hover:border-primary/30 transition-all duration-300 overflow-hidden h-full">
                    {/* Glow blob */}
                    <div className={`absolute -top-10 -right-10 w-40 h-40 rounded-full blur-3xl opacity-0 group-hover:opacity-30 transition-opacity duration-500 ${gradient}`} />
                    ...
                    <div className={`inline-flex p-3.5 rounded-2xl mb-5 ${gradient} bg-opacity-10`}>
    ...
    const features = [
        {
            ...
            gradient: 'bg-blue-500 text-blue-500',
        ...
        {
            icon: Clock, to: '/focus',
            gradient: 'bg-primary text-primary',
        ...
        },
        {
            ...
            gradient: 'bg-teal-500 text-teal-500',
            badge: 'Markdown',
        ...
        },
        {
            ...
            gradient: 'bg-purple-500 text-purple-500',
            badge: lang === 'id' ? 'Mingguan' : 'Weekly',
            title: t('home.feat4.title'),
            desc: t('home.feat4.desc'),
        },
        {
            icon: BarChart2, to: '/progress',
            gradient: 'bg-green-500 text-green-500',
            badge: lang === 'id' ? 'Real-time' : 'Real-time',
            ...
            <section className="relative text-center pt-8 pb-20 overflow-hidden">
                ...
                >
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        <PillStat value="5" label={lang === 'id' ? 'Alat Produktivitas' : 'Productivity Tools'} />
                        <PillStat value="100%" label={lang === 'id' ? 'Offline & Privat' : 'Offline & Private'} />
                        <PillStat value="0ms" label={lang === 'id' ? 'Waktu Loading' : 'Loading Time'} />
                        <PillStat value="∞" label={lang === 'id' ? 'Gratis Selamanya' : 'Free Forever'} />
                    </div>
                </motion.div>
            </section>
            ...
            <section className="pb-20">
                <FadeUp className="text-center mb-12">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 font-bold text-xs uppercase tracking-widest mb-4">
                        ...
                        {lang === 'id' ? '5 Fitur Produktivitas' : '5 Productivity Features'}
                    </div>
                    ...
                    <FadeUp>
                        ...
                        <div className="space-y-6">
                            <Step number="1" delay={0.1}
                                title={lang === 'id' ? 'Tambahkan tugas & deadline' : 'Add tasks & deadlines'}
                                desc={lang === 'id' ? 'Atur dengan Eisenhower Matrix — apa yang mendesak, apa yang penting.' : 'Organize with Eisenhower Matrix — what\'s urgent, what\'s important.'} />
                            <Step number="2" delay={0.2}
                                title={lang === 'id' ? 'Fokus dengan Pomodoro' : 'Focus with Pomodoro'}
                                desc={lang === 'id' ? 'Timer 25 menit + suara ambient generatif. Lacak sesi fokusmu tiap hari.' : '25-minute timer + generative ambient sounds. Track your daily focus sessions.'} />
                            <Step number="3" delay={0.3}
                                title={lang === 'id' ? 'Lacak progresmu' : 'Track your progress'}
                                desc={lang === 'id' ? 'Lihat perkembangan 7 hari terakhir dalam grafik yang jelas dan motivatif.' : 'View your last 7 days of progress in clear, motivating charts.'} />
                        </div>
                    </FadeUp>
                        ...
                        <div className="relative">
                            ...
                            <div className="relative bg-bg-card border border-border/60 rounded-3xl p-8 shadow-xl">
                                ...
                                <div className="space-y-3 mb-6">
                                    {[
                                        { done: true, text: lang === 'id' ? 'Review materi Algoritma' : 'Review Algorithm materials', color: 'text-green-500' },
                                        { done: false, text: lang === 'id' ? 'Kerjakan tugas basis data' : 'Complete database assignment', color: 'text-text-main' },
                                        { done: false, text: lang === 'id' ? 'Baca jurnal penelitian' : 'Read research journal', color: 'text-text-muted' },
                                    ].map((item, i) => (
                                ...
                                <div>
                                    <div className="flex justify-between text-xs font-bold mb-2">
                                        <span className="text-text-muted">{lang === 'id' ? 'Progres Hari Ini' : "Today's Progress"}</span>
                        ...
                        {[
                            {
                                ...
                                title: lang === 'id' ? '100% Privat' : '100% Private',
                                desc: lang === 'id' ? 'Data tidak pernah meninggalkan browsermu. Tidak ada server, tidak ada akun.' : 'Data never leaves your browser. No server, no account required.',
                            },
                            {
                                icon: Zap,
                                color: 'text-amber-500 bg-amber-500/10',
                                title: lang === 'id' ? 'Super Cepat' : 'Blazing Fast',
                                desc: lang === 'id' ? 'Tidak ada loading dari server. Semua instan, kapan saja, di mana saja.' : 'No server round-trips. Everything is instant, anytime, anywhere.',
                            },
                            {
                                icon: Star,
                                color: 'text-primary bg-primary/10',
                                title: lang === 'id' ? 'Dua Bahasa' : 'Bilingual',
                                desc: lang === 'id' ? 'Tampil dalam Bahasa Indonesia dan Inggris. Toggle kapan saja.' : 'Available in Indonesian and English. Toggle anytime.',
                            },
                        ].map((item, i) => (
                    ...
                    <div className="relative bg-gradient-to-br from-primary via-indigo-600 to-secondary rounded-3xl p-12 text-center text-white overflow-hidden shadow-2xl shadow-primary/30">
                        ...
                        <div className="relative z-10">
                            ...
                            <h2 className="text-4xl font-extrabold mb-4">
                                {lang === 'id' ? 'Siap untuk lebih produktif?' : 'Ready to be more productive?'}
                            </h2>
                            <p className="text-white/70 text-lg mb-8 max-w-md mx-auto">
                                {lang === 'id' ? 'Mulai sekarang. Gratis. Selamanya.' : 'Start now. Free. Forever.'}
                            </p>
                            <Link
                                to="/tasks"
